...
- 🌐 **多语言交互** - 完整的中英文交互式向导
- 🧠 **路由器集成** - 通过 claude-code-router 启用思考能力
- 🔀 **本地路由服务** - 支持 localhost:3456 路由中间层
- ⚡ **思考模式** - 通过路由器中间层转换请求格式启用推理
...
# 路由器状态检查
claude-switch router-status        # 检查路由器安装和运行状态
...
### v2.3.0 (最新)
- ✨ **新增router-status命令** - 路由器安装和运行状态检查
- ✨ **新增路由器配置模板** - 支持GLM、DeepSeek、MiniMax的思考模式配置
- 🔀 **路由器集成** - 支持通过claude-code-router启用API思考能力
- 🧠 **思考模式支持** - 通过路由器中间层转换请求格式，启用推理和反思能力
- 📡 **本地路由服务** - 配置指向localhost:3456路由服务，路由器负责添加enable_thinking参数
- 🔧 **无缝切换** - 与现有配置系统完全兼容，可通过配置名直接切换到路由器模式
